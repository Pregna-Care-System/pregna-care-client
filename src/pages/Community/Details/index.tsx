  ...
  const [comments, setComments] = useState([
    {
      ...
      content:
        'I completely agree with Sarah! Staying hydrated made a huge difference for me during my first trimester.',
    ...
    <div className='max-w-4xl mx-auto p-4 bg-gradient-to-b from-pink-50 to-white min-h-screen'>
      ...
      <div className='bg-white rounded-xl shadow-lg p-6 mb-6'>
        <div className='flex items-start space-x-4 mb-6'>
          <img src={comment.user.avatar} alt={comment.user.name} className='w-12 h-12 rounded-full object-cover' />
          <div className='flex-1'>
            <div className='flex items-center space-x-2'>
              <h2 className='font-semibold text-gray-800'>{comment.user.name}</h2>
              <MdVerified className='text-blue-500' />
              <span className='text-sm text-pink-600 bg-pink-50 px-2 py-1 rounded-full'>{comment.user.expertise}</span>
            </div>
